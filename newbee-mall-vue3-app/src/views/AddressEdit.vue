...
onMounted(async () => {
  ...
  if (type == 'edit') {
    ...
    /**
     * 这段代码通过遍历state.areaList.county_list数组，找到与传入的地址详细信息匹配的区域代码，并将结果赋值给_areaCode变量
     */

    //使用Object.entries()方法将state.areaList.county_list对象转换为数组
...
const onSave = async (content) => {
  ...
  // 如果state的type属性为'edit'，则将state的addressId属性赋值给params的addressId属性。 
  if (state.type == 'edit') {
